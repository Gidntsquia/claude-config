...
#
# Install, per repo: copied to <repo>/.claude/hooks/full-suite-guard.sh and
# registered as a PreToolUse hook with matcher "Bash" in
# <repo>/.claude/settings.json.
#
# Install, global: the same hook entry in ~/.claude/settings.json pointing at
# ~/.claude/hooks/full-suite-guard.sh. Safe to leave on everywhere, because
# <repo>/.claude/test-commands.sh is the activation signal — see below.
#
...
# five minutes this skill exists to save — both conditional on the repo opting
# in at all. Matching is anchored at the start of a command segment, so prose
# mentioning a runner never trips it.
set -uo pipefail
...
# The per-repo commands file is also the on switch. A repo that has not been
# through this skill has no narrow command to offer, so blocking its test runs
# would be pure obstruction — exit and let it through. This is what makes a
# global install safe: the guard is inert everywhere except repos that were
# actually tiered, and it turns itself on the moment one is.
FILE_CMD=""; CHANGED_CMD=""; FAST_CMD=""; FULL_CMD=""; DEFAULT_IS_FAST=0
found=0
for f in "${CLAUDE_PROJECT_DIR:-.}/.claude/test-commands.sh" .claude/test-commands.sh; do
  # shellcheck disable=SC1090
  if [ -f "$f" ]; then . "$f"; found=1; break; fi
done
[ "$found" = 1 ] || exit 0
