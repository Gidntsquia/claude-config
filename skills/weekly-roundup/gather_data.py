...
# Per-million-token USD list prices. Update as pricing changes.
PRICING = {
    "claude-sonnet-5":        {"in": 3.00,  "out": 15.00, "display": "Sonnet 5"},
    "claude-opus-5":          {"in": 15.00, "out": 75.00, "display": "Opus 5"},
    "claude-haiku-4-5-20251001": {"in": 1.00, "out": 5.00, "display": "Haiku 4.5"},
    "claude-fable-5-1":       {"in": 10.00, "out": 50.00, "display": "Fable 5.1"},
}
...
def scan_project_dir(dir_path: Path, window_start: datetime):
    """Return {model: {input, output, cache_read, cache_5m, cache_1h}} for one
    Claude-project jsonl directory, deduped by message id."""
    totals = {}
        ...
        for line in lines:
            ...
            if msg_id and key in seen:
                ...
                if out_t > prev_out:
                    ...
                    seen[key] = out_t
                    slot = totals.setdefault(model, {"input": 0, "output": 0, "cache_read": 0, "cache_5m": 0, "cache_1h": 0})
                    slot["output"] += delta
                continue
            if msg_id:
                seen[key] = out_t
    ...
    for repo in repos:
        ...
        for d in claude_projects.iterdir():
            ...
            if d.name == expected_slug or d.name.startswith(expected_slug + "-"):
                ...
                dir_totals = scan_project_dir(d, token_window_start)
